...
install:
	docker run -it --rm -v ${PWD}:/app ${DOCKER_IMAGE} npm install

start:
	docker run -it --rm -v ${PWD}:/app -p "3030:1042" ${DOCKER_IMAGE} npm start   

release:
	docker run -it --rm -v ${PWD}:/app -p "3030:1042" ${DOCKER_IMAGE} npm run release 
